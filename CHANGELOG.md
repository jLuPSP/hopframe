...
- **Gateway data-plane surfaces.** Two new ways to run Hopframe over the same detection pipeline. `mcp-extauthz` is an Envoy HTTP external-authorization adapter, so any Envoy-based gateway (Envoy, Istio, Gloo, Emissary) can get an allow/deny decision on inbound MCP with no per-gateway code. `mcp-gateway` is a native multiplexer that fronts many MCP upstreams at one address with full inline fidelity and shared quarantine/taint state. See [docs/deploy.md](docs/deploy.md) for the deploy paths and `deploy/labs/extauthz-e2e/` for a real-Envoy end-to-end lab.
