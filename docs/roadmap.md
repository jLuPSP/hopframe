...
This document defines what Hopframe is building and the build order: the next 12 months at a solo nights-and-weekends pace. Work and design-partner feedback will reshape the priorities.

## Status snapshot

The first delivery cycle landed the core of all three pillars in v0.1. What remains is polish on shipped work, plus deferred work that needs real infrastructure and design-partner input: HA Postgres, OIDC integration testing against a real IdP, SOC 2 Type II, and cryptographic per-tenant scoping.
...
2. **Enterprise-grade management and control plane.** This includes RBAC, SSO, multi-tenant administration, sensor fleet inventory, HA on a real database, and SOC 2 Type II for the managed offering.
3. **Cryptographic audit-grade evidence.** This includes a hash-chained log, signed exports with chain proofs, externally witnessed timestamps, per-record signing for selective disclosure, and forensic export shapes that SOC 2 and HIPAA auditors accept.

Closed competitors (Bedrock Guardrails, Model Armor) do parts of pillar 1 inside their own cloud, lock customers to a vendor, and do not speak MCP/A2A semantics. Open competitors (NeMo Guardrails, Garak, LLM-Guard) provide parts of pillar 1 without an enterprise control plane or cryptographic audit. MCP gateways handle routing.
...
[Policies](policies.md) bind rule matches to a scope and response. This pillar tracks the remaining policy work.

Today Hopframe ships rules in YAML under `content/<category>/`, distributes them via the Helm chart, and reloads them on sensor restart. Operators can override the bundle by mounting a different ConfigMap. That is configuration; editable enterprise policy requires a separately addressable resource operators can change through the control plane.
...
Today the control plane is a single-process Go binary with bearer-token auth, per-tenant token scoping, Prometheus `/metrics`, optional per-IP rate limiting, and an embedded web UI. Enterprise operators expect a different surface.
...
- **Sensor fleet inventory.** The inventory shows every connected sensor with version, active policy version, and last heartbeat. It supports force-update and configuration-drift detection: "sensor S is running policy v17 but the active version is v19."
...
- **SOC 2 Type II for the managed offering.** This requires auditor engagement, a control catalog, and evidence collection; it takes months and is gated on the first paying design partner.
...
Today, a hash-chained NDJSON log lives under `control-plane/store/`. `/v1/verify` walks the chain on demand. Signed CSV and NDJSON exports carry a chain-proof trailer bound to the head hash at export time. Enterprise compliance teams want more.
...
Each quarter should produce a deployable result that a design partner can use.

**Months 1 through 3.** Phase 1A + 1B + 1C: policy CRUD, distribution, hierarchy. Plus Phase 3D: forensic-export CLI. This supports a design partner conversation about running the product.

**Months 4 through 6.** Phase 2A: RBAC + OIDC. Phase 1D: dry-run / preview. Phase 3A: Rekor anchoring. At the end, a regulated buyer can install Hopframe, configure policies through SSO with role-scoped tokens, preview changes before applying them, and produce externally witnessed audit evidence.
...
The sequence defers the most expensive pieces (HA migration, SOC 2) until after design-partner validation, avoiding a quarter spent on infrastructure that nobody requested.

## What this roadmap is not

- **Solo scale.** If a design partner does not materialize after Months 1 through 6, slow down, talk to more buyers, or pivot. Do not ship more code.
...
- **Three-pillar focus.** Anything that does not advance a pillar is a distraction in this window. Detection content expansion (on the v0.1 gap list) is in scope when it strengthens pillar 1.

## Out of this roadmap

These appear elsewhere on the v0.1 gap list and are not part of the three-pillar core:

- The TypeScript / JavaScript SDK reaches into the Node MCP ecosystem; strategic and separable from the three pillars, it gets its own track.
- Public-corpus benchmarks (HarmBench, JailbreakBench, AgentDojo) provide credibility content and run in parallel.
- The Layer 3 LLM judge is an optional detector and runs in parallel.
