...
When Agent Engine calls MCP tools over HTTP, point the tool URLs at a Hopframe sensor in your VPC.
...
The sensor is the existing `mcp-sensor` binary, deployed via the Helm chart at `deploy/helm/hopframe/`. Agent Engine needs no code changes, only a tool configuration change.
...
3. **Update Agent Engine tool config** to point at the sensor's `ClusterIP` service. With LangChain's MCP tooling, the tool URL is what changes:
...
5. **Deploy the agent** to Agent Engine as usual.

The detection pipeline, taint tracking, quarantine, and signed audit log work without further changes.

---

## Shape 2: Python SDK path

When your agent calls tools that are not MCP (direct Python functions, REST APIs, custom tool classes), the wire path cannot see them. Use the Python SDK to emit events from LangChain or LangGraph callbacks.
...
1. Until the first PyPI release, install the SDK from the repository:

   ```text
   # requirements.txt
   hopframe[langchain] @ git+https://github.com/jLuPSP/hopframe.git@main#subdirectory=sdk/python
   ```
...
5. **Deploy and run.** Events from Python tool calls land on the Hopframe timeline, alongside any MCP traffic from Shape 1.

You can mix both shapes. MCP tools go through the sensor; Python tools emit via the SDK. When both share the same `agent_run_id`, the Hopframe UI shows one forensic timeline.
...
With either shape wired, every Agent Engine deployment that uses your Hopframe-equipped MCP servers (or the SDK) provides:
...
**No `agent_run_id` showing up.** Set `X-Hopframe-Agent-Run-Id` (HTTP path) or pass `run_id` to `HopframeCallback` (SDK path). Without it, every request mints a fresh synthetic id and you lose correlation.
