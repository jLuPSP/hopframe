...
Hopframe lives at the protocol layer of agent traffic, inline on the MCP and A2A wire. Most adjacent tools live at the model layer or the routing layer.

This map draws on each tool's public docs, open-source code, and product pages as of mid-2026, cross-checked against the research in [landscape-research.md](landscape-research.md). I flag claims that rest only on vendor marketing and claims I could not verify.

## The four layers

- **Model-boundary guardrails** inspect the LLM prompt and response: Bedrock Guardrails, Model Armor, Lakera Guard, NeMo Guardrails. They never see MCP or A2A protocol content, because it is not a prompt.
...
- **Inline protocol-wire inspection** reads live JSON-RPC and A2A traffic and acts on its content. This is Hopframe's primary layer; the one credible open peer is Solo.io agentgateway.
...
The tools overlap only at the edges. Model-boundary filters inspect the prompt and response inside the model call; none sit between an MCP server and the agent, so a poisoned tool description can enter the model's context unseen. The one overlap, layer-1 regex on tool-result text, is detailed in the last section. Running both covers both layers.

### We use Bedrock Agents or OpenAI Assistants and cannot run a sidecar in the agent's runtime.

Hopframe deploys at the protocol counterparty you control. If the agent calls an MCP server you host, put the sensor in front of that server; it inspects the traffic crossing your network, regardless of the agent's runtime. When one cloud vendor manages both endpoints and the traffic never crosses your network, nobody can intercept it; pair Hopframe with that vendor's first-party guard there.

### We already have a SIEM.

The signed-webhook and Splunk HEC exporters ship findings into it. SIEMs do not parse MCP or A2A semantics on their own; Hopframe supplies the protocol-level findings.
...
- **Detection depth.** agentgateway's MCP inspection is gateway and policy oriented (auth, RBAC, fingerprinting, CEL rules). Hopframe runs a four-stage content pipeline with regex, a heuristic feature-density classifier, an optional LLM judge for the uncertain band, and behavioral anomaly detection, with NFKC normalization and base64 decoding before matching.
...
Read this rough map with the caveats above. Several cells rest on marketing rather than tested mechanisms, and coverage of the fast-moving startups is incomplete. Corrections are welcome via PR.
...
The honest overlap with model-layer filters is layer-1 prompt-injection regex on tool-argument and tool-result text. Both Lakera and Hopframe will catch "ignore previous instructions" in a tool return, at different moments and with different metadata:

- A model-layer filter sees the string in the prompt, after the agent has built it. It can block the prompt but does not know which tool produced the string.
- Hopframe sees the string on the tool's response wire, before it enters the prompt. It can quarantine the tool itself, mark the counterparty as risky, propagate taint into A2A, and stamp the rule's content hash onto the finding for reproducibility.

Running both gives coverage at both layers plus a chain-of-custody audit trail the model-layer tools do not produce.
