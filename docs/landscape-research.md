...
Cited research backing the Hopframe case study. Calibrated, not promotional.
Generated via a multi-agent deep-research pass (5 angles, 23 sources fetched, 113 claims
...
tool was found doing all of: inline MCP+A2A wire detection in-environment, cross-protocol
...
medium confidence — bounded by the tools surveyed.)

## The honest recalibration (read this before writing claims)

**"Nothing sits inline on the MCP/A2A wire" is FALSE.** Solo.io's **agentgateway** does, and
it is credible: open-source (Apache 2.0, Rust, Linux Foundation), drops in with no code changes
(Ambient waypoint), proxies BOTH MCP and A2A, and inspects MCP tool calls beyond prompts
...
**What still holds against agentgateway:**
- Its detection is gateway/policy-grade (auth, RBAC, fingerprinting, CEL policy), not
  Hopframe's four-stage content detection (regex -> heuristic density -> LLM judge ->
  behavioral, with NFKC/base64 normalization).
- Its LLM guardrails (prompt-injection/data-leak) run ONLY on the client<->LLM prompt/response
  channel, not on MCP tool-description/result or A2A protocol content.
...
| **Invariant MCP-Scan** | static scan | MCP tool descriptions/names only; not results or A2A | Open, but traffic can leave env (Guardrails API default) | No |
...
HiddenLayer, Protect AI. Treat as "not evaluated," not "no peer."
...
Plus the full **hash-chain + Ed25519 + Rekor** audit log, which no surveyed tool ships
...
- **arXiv:2602.11327** (Apr 2026, Canadian Institute for Cybersecurity + Mastercard) — MCP has
  ...
  relay-abuse risk; calls for cross-protocol standards. Substantiates the cross-protocol motivation.
- **arXiv:2603.24775** (AIP) — the rare cross-protocol identity work; confirms most solutions
